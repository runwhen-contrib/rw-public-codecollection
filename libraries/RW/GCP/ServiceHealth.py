...
class ServiceHealth:
    ...
    def filter_status_results(
        ...
        - ``seconds_in_past`` how much history will be kept during filtering.
        ...
        Examples:
        | RW.GCP.ServiceHealth.Filter Status Results   |   ${history}   |   ${seconds_in_past}  |
        | RW.GCP.ServiceHealth.Filter Status Results   |   ${history}   |   ${seconds_in_past}      |   products=${PRODUCT_CSV}  | regions=${REGIONS_CSV}   |
        ...
        matches = []
        history = self.filter_history_by_time(history, seconds_in_past)
        for entry in history:
            ...
            if check_ongoing:
                ongoing = self._is_incident_ongoing(entry, seconds_in_past)
            else:
                ongoing = True
            if (
    ...
    def _is_incident_ongoing(self, entry, seconds_in_past) -> bool:
        ongoing = False
        now = datetime.now().replace(tzinfo=None)
        datetime_in_past = datetime.now() - timedelta(seconds=int(seconds_in_past))
        entry_start = dateutil.parser.parse(entry["begin"]).replace(tzinfo=None)
        if "end" in entry and entry["end"]:
            entry_end = dateutil.parser.parse(entry["end"]).replace(tzinfo=None)
            if (
    ...
    def filter_history_by_time(self, history, seconds_in_past):
        """
        Helper method for removing history entries not within provided seconds range.
        """
        datetime_in_past = (datetime.now() - timedelta(seconds=int(seconds_in_past))).replace(tzinfo=None)
        filtered_history = []
        for entry in history:
            entry_start_datetime = dateutil.parser.parse(entry["begin"]).replace(tzinfo=None)
