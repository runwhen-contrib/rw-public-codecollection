...
class Papi:
    ...
    def authenticate(self, username="", password="", token=None, base_url=None):
        """
        Requests an access token for the RunWhen Public API.

        This keyword will leverage platform configuration when needed.
        ...
        |   token  |
        """
        if not username and "RW_USERNAME" in os.environ: # refactor:
            username = self._core.import_platform_variable("RW_USERNAME")
        if not password and "RW_PASSWORD" in os.environ:
            password = self._core.import_platform_variable("RW_PASSWORD")
        if not token and "RW_ACCESS_TOKEN" in os.environ: # refactor
            token = self._core.import_platform_variable("RW_ACCESS_TOKEN")
        if not base_url:
            base_url = self._core.import_platform_variable("RW_API_BASE_URL")

        if not base_url:
            raise ValueError(f"The base API URL was not set to a valid value: {base_url}")
        self.base_url = base_url
        if token:
            self.token = token
        if not self.token:
            papi_session = requests.Session()
            ...
            if not rsp.status_code == 200:
                raise AssertionError(
                    f"Authenticating to {token_path} with username {username} did not result in http 200, but got: {rsp}\n{rsp.content}"
                )
            self.token = rsp.json()["access"]
            return self.token
        return self.token
    ...
    def get_all_recents_in_workspace(self, workspace, history: str = "5m", resolution: str = "30s"):
        ...
        for sli in self.get_slis(workspace, names_only=True):
            all_recents[sli] = self.get_sli_recent(workspace, sli, values_only=True, history=history, resolution=resolution)
        return all_recents
    ...
    def get_runrequest_report(self, workspace=None, slx=None, runrequest_id=None, results_only=True, template="basic_str_template"):
        """
            ...
            for rr in rsp["runRequests"]:
                ...
                if slx not in this_slx:
                    ...
                    if not response_time:
                        ...
                        report = self.get_runrequest_report(workspace, slx, runrequest_id=runrequest_id, template=template)
    ...
    def get_runsession_url(self, workspace=None, runsession_id=None, runrequest_id=None):
        ...
        rsp = None
        if runsession_id and runrequest_id and workspace:
            session = self._get_session()
            url = f"{self.base_url}{self.base_path}{workspace}/runsessions/{runsession_id}"
            rsp = session.get(url)
            if rsp.status_code == 404:
        ...
        if "name" in rsp:
            params = {"cmd": f"get rr/{runrequest_id}"}
